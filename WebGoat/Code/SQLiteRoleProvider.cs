			...
			try {
				using (SqliteCommand cmd = cn.CreateCommand()) {
					cmd.CommandText = String.Format(
						"SELECT r.RoleName FROM {0} r INNER JOIN {1} uir ON r.RoleId = uir.RoleId INNER JOIN {2} u ON uir.UserId = u.UserId WHERE (u.LoweredUsername = $Username) AND (u.ApplicationId = $MembershipApplicationId)",
						ROLE_TB_NAME, USERS_IN_ROLES_TB_NAME, USER_TB_NAME);
